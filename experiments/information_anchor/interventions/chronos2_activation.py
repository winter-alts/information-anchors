...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Chronos2 frozen-backbone activation replacement on held-out forecasts"
    )
...
def _median_index(model: Any) -> int:
...
def _forecast_batch(
    ...
    context = torch.from_numpy(histories).to(device=device, dtype=model.dtype)
    outputs = model(
        context=context,
    ...
    prediction = outputs.quantile_preds[:, _median_index(model), :pred_len]
...
def _replace_block_hidden(output: Any, hidden: torch.Tensor) -> Any:
    return type(output)(
...
def _capture_layer_outputs(
    ...
    def capture_hook(_module, _inputs, output):
        captures.append(_block_hidden(output).detach().clone())
        return output

    handle = model.encoder.block[layer].register_forward_hook(capture_hook)
    try:
        _forecast_batch(model, histories, pred_len, num_output_patches, device)
    finally:
        handle.remove()
    if len(captures) != 1:
...
def _patched_forecast_batch(
    ...
    def replacement_hook(_module, _inputs, output):
        ...
        call_index += 1
        return _replace_block_hidden(output, changed)

    handle = model.encoder.block[layer].register_forward_hook(replacement_hook)
    try:
        forecast = _forecast_batch(model, histories, pred_len, num_output_patches, device)
    finally:
        handle.remove()
...
def _noop_clone_forecast(
    ...
    def clone_hook(_module, _inputs, output):
        return _replace_block_hidden(output, _block_hidden(output).clone())

    handle = model.encoder.block[layer].register_forward_hook(clone_hook)
    try:
        return _forecast_batch(model, histories, pred_len, num_output_patches, device)
    finally:
        handle.remove()
...
def main() -> None:
    ...
    if config.model.name != "Chronos2":
        raise ValueError("This intervention runner supports Chronos2 only.")
    ...
    num_output_patches = math.ceil(config.data.pred_len / model.chronos_config.output_patch_size)
    baseline_parts = []
    with torch.no_grad():
    ...
    output_dir = Path(args.output_root) / f"chronos2_l{config.data.seq_len}_p{config.data.pred_len}_{timestamp}"
    ...
    payload = {
        ...
        "intervention": "Replace one post-block context token with the same token from a temporally distant donor; frozen later blocks, final norm, and quantile head remain unchanged.",
        "point_forecast": "Checkpoint median (0.5) quantile.",
